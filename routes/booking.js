...
app.get("/", async (req, res) => {
  ...
  try {
    if (branch_id) bookings = await Booking.find({ branch_id }).select("-password");
    else if (cinema_id) bookings = await Booking.find({ cinema_id }).select("-password");
    else if (theater_id) bookings = await Booking.find({ theater_id }).select("-password");
    else  bookings = await Booking.find({ cinema_id,branch_id, theater_id }).select("-password");
...
app.post("/", async (req, res) => {
  try {
    ...
    if(!seat) return res.json({msg:"Invalid seat number or theater was passed"})
